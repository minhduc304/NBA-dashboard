...
class PropOutcomeTracker:
    ...
    def backfill_all(self, verbose: bool = False) -> Dict[str, int]:
        ...
        logger.info("Found %d dates with props to process", len(dates))
        ...
        logger.info(
            "Backfill complete: %d matched, %d no game log, %d unsupported, %d errors",
            totals['matched'], totals['no_game_log'], totals['unsupported_stat'], totals['errors']
        )
    ...
    def backfill_odds_api_props(self, verbose: bool = False) -> Dict[str, int]:
        ...
        logger.info("Found %d dates with odds_api props to process", len(dates))
        ...
        logger.info(
            "Odds API backfill complete: %d matched, %d no game log, %d unsupported, %d errors",
            totals['matched'], totals['no_game_log'], totals['unsupported_stat'], totals['errors']
        )
    ...
    def print_statistics(self):
        """Log statistics about prop outcomes."""
        ...
        if stats['total_outcomes'] > 0:
            ...
            logger.info(
                "PROP OUTCOMES: %d total (%s to %s), Over=%.1f%%, Under=%.1f%%, Avg Edge=%+.2f",
            ...
            logger.info("PROP OUTCOMES: No outcomes tracked yet")
    ...
    def print_unmatched_names(self, limit: int = 50):
        """Log unmatched prop names for review."""
        unmatched = self.find_unmatched_prop_names(limit)

        if unmatched:
            logger.info("UNMATCHED NAMES: %d players need aliases", len(unmatched))
            for name, count in unmatched[:10]:
                logger.info("  %s (%d props)", name, count)
        else:
            logger.info("UNMATCHED NAMES: All player names matched")
    ...
    def seed_aliases_from_corrections(self) -> int:
        ...
        logger.info("Seeded %d aliases from NAME_CORRECTIONS", inserted)
        return inserted
    ...
    def add_alias(self, alias: str, canonical_name: str, source: str = 'manual') -> bool:
        ...
        if not result:
            conn.close()
            logger.warning("Player not found: %s", canonical_name)
            return False
        ...
        try:
            ...
            if success:
                logger.info("Added alias: '%s' -> '%s'", alias, canonical_name)
            return success
        except sqlite3.Error as e:
            conn.close()
            logger.error("Error adding alias: %s", e)
            return False
...
def main():
    ...
    logger.info("Processing props from all sources...")
    totals = tracker.backfill_all(verbose=False)

    # Process Odds API props (separate table)
    logger.info("Processing Odds API props...")
    odds_totals = tracker.backfill_odds_api_props(verbose=False)

    # Summary
    logger.info(
        "COMPLETE: All Sources=%d matched, Odds API=%d matched",
        totals['matched'], odds_totals['matched']
    )
